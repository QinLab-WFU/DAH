...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.identity_map_reordering = identity_map_reordering
        self.mhatt = MultiHeadGeometryAttention(
            d_model,
            d_k,
            d_v,
            h,
    ...
    def forward(
        self, queries, keys, values, relative_geometry_weights, attention_mask=None, attention_weights=None, pos=None
    ):
...
def BoxRelationalEmbedding(f_g, dim_g=64, wave_len=1000, trignometric_embedding=True):
    ...
    assert seq_len in [196, 49]
    grid_size = (7, 7) if seq_len == 49 else (14, 14)
    x_min, y_min, x_max, y_max = get_grids_pos(batch_size, seq_len, grid_size)
    x_min, y_min, x_max, y_max = x_min.to(f_g.device), y_min.to(f_g.device), x_max.to(f_g.device), y_max.to(f_g.device)
    cx = (x_min + x_max) * 0.5
    cy = (y_min + y_max) * 0.5
    w = (x_max - x_min) + 1.0
    h = (y_max - y_min) + 1.0
...
class MultiLevelEncoder_woPad(nn.Module):
    ...
    def forward(self, input, attention_mask=None, attention_weights=None, pos=None):
        ...
        relative_geometry_weights = F.relu(relative_geometry_weights)  # maybe Eq. 4
        out = input
...
class TransformerDecoderLayer(nn.TransformerDecoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", SAtt=True):
        super().__init__(d_model, nhead, dim_feedforward=dim_feedforward, dropout=dropout, activation=activation)
        self.SAtt = SAtt
...
class Transformer(nn.Module):
    def __init__(
        ...
        # input embedding
        self.embed_cv = nn.Linear(in_dim_cv, dim_com)
        self.embed_attr = nn.Linear(in_dim_attr, dim_com)

        # transformer encoder
        self.transformer_encoder = MultiLevelEncoder_woPad(
            N=ec_layer, d_model=dim_com, h=1, d_k=dim_com, d_v=dim_com, d_ff=dim_feedforward, dropout=dropout
        )

        # transformer decoder
        decoder_layer = TransformerDecoderLayer(
            d_model=dim_com, nhead=heads, dim_feedforward=dim_feedforward, dropout=dropout, SAtt=SAtt
        )
    ...
    def forward(self, f_cv, f_attr):
        ...
        h_cv = self.embed_cv(f_cv.permute(0, 2, 1))  # B x C x H*W -> B x H*W x C -> B x H*W x 300
        h_attr = self.embed_attr(f_attr)  # A x 300 -> A x 300
        h_attr_batch = h_attr.unsqueeze(0).repeat(f_cv.shape[0], 1, 1)  # A x 300 -> B x A x 300
